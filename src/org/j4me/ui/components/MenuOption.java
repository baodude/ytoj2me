 ...
 * <p>
 * The default implementation shows the choice number on the left, then the text
 * for the choice, and an arrow on the right if it is a submenu. The text is
 * clipped if it runs over a single line. This keeps all menu items the same
 * height.
 * 
 * @see Menu
 */
public class MenuOption extends Component {
	 ...
	 * the left and right edges of the component. Increasing this number reduces
	 * the width of usuable space to write the menu choice's text.
	 */
	...
	/**
	 * The menu text to use if explicitly set. This can be <code>null</code> in
	 * which case <code>menuItem.getText()</code> or
	...
	/**
	 * When <code>true</code> it indicates this option is for a submenu of the
	 * current menu. Submenus have an arrow that appears to the right of the
	 * option text. There is nothing else special about them.
	 */
	 ...
	 *            is the command that is represented by this component.
	 * @param submenu
	 *            when <code>true</code> indicates this is a submenu of the
	 *            current menu.
	 */
	 ...
	 *            <code>null</code> the <code>MenuItem.getText()</code> or
	...
	/**
	 * Paints a <code>MenuOption</code>. On the left is the choice number in a
	 * box. The middle has the text for the choice. If it is a submenu the right
	 * has an arrow.
	 * 
	 * @param g
	 ...
	 * @param selected
	 *            is <code>true</code> if this is the currently highlighted menu
	 *            choice.
	 * 
	...
	protected void paintComponent(Graphics g, Theme theme, int width,
		...
		if (submenu) {
			...
			int arrowColor = selected ? theme.getMenuFontColor() : theme
					.getFontColor();
			g.setColor(arrowColor);

			// Draw the arrow as a triangle.
			g.fillTriangle(arrowX, arrowY, arrowX, arrowY + arrowHeight, arrowX
					+ arrowWidth, arrowY + arrowHeight / 2 + 1);
		}
	}
	 ...
	 */
	protected void paintText(Graphics g, Theme theme, int x, int y, int width,
			int height, boolean selected) {
		// Set the font color.
		int fontColor = selected ? theme.getBackgroundColor() : theme
				.getFontColor();
	...
	/**
	 * Returns the size of the menu choice. It will be one line of text high and
	...
	public int[] getPreferredComponentSize(Theme theme, int viewportWidth,
		...
		int[] submenuDimensions = getSubmenuIndicatorSize(theme, viewportWidth,
				viewportHeight);

		// Calculate the width of the area the text can appear in.
		int textWidth = viewportWidth - 3 * HORIZONTAL_MARGIN
				- submenuDimensions[0];

		// Calculate the dimensions of the text.
		int[] textDimensions = getPreferredTextSize(theme, textWidth,
				viewportHeight);
	 ...
	 * @return An array with two elements where the first is the width in pixels
	 ...
	 *         and the second is the height.
	 */
	protected int[] getSubmenuIndicatorSize(Theme theme, int viewportWidth,
			int viewportHeight) {
		// Get the dimensions of the arrow.
