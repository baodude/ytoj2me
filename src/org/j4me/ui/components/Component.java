 ...
 * include labels, text boxes, and check boxes. The <code>Dialog</code> class
 * calls methods on this interface to layout, paint, and manage components.
 */
public abstract class Component {
	 ...
	 * <code>Graphics.LEFT</code>, <code>Graphics.HCENTER</code>, and
	 ...
	 * (0,0) and the component fills the rectangle bounded by <code>width</code>
	...
	public final void paint(Graphics g, Theme theme, DeviceScreen screen,
		...
		if (isShown()) {
			...
			g.clipRect(0, 0, width, height * 2); // *2 to workaround clipping
			// bug
	 ...
	 * the time of the call, the <code>Graphic</code>s context's destination is
	 * the content area of this <code>Component</code> (or back buffer for it).
	 * The translation is set so that the upper left corner of the content area
	 * is at (0,0), and the clip is set to the area to be painted. The
	 * application must paint every pixel within the given clip area. The item
	 * is allowed to modify the clip area, but the system must not allow any
	 * modification to result in drawing outside the bounds of the item's
	 * content area. The <code>w</code> and <code>h</code> passed in are the
	 * width and height of the content area of the item. These values will
	 * always be set to the clip width and height and are passed here for
	 * convenience.
	 * <p>
	 ...
	 *            selected and <code>false</code> when it is not.
	 */
	protected abstract void paintComponent(Graphics g, Theme theme, int width,
	 ...
	 */
	public final int[] getPreferredSize(Theme theme, int viewportWidth,
			int viewportHeight) {
		// Get the component's dimensions.
		int[] dimensions = getPreferredComponentSize(theme, viewportWidth,
				viewportHeight);
	 ...
	 * cannot be wider than the screen or it will be cropped. However, it can be
	 * taller than the screen, in which case a scroll bar will be added to the
	 * form this component resides on.
	 * 
	 * @param theme
	 *            is the application's <code>Theme</code>.
	 * @param viewportWidth
	 *            is the width of the viewable area, in pixels, the component
	 *            can use.
	 * @param viewportHeight
	 *            is the height of the viewable area, in pixels, the component
	 *            can use.
	 * @return A array with two elements where the first is the width of the
	 *         component in pixels and the second is the height.
	 */
	public abstract int[] getPreferredComponentSize(Theme theme,
			int viewportWidth, int viewportHeight);

	/**
	 * Tells if this component accepts user input or not. If it does then it can
	 * be scrolled to by the user. If it does not, it will be displayed, but can
	 * be skipped over by scrolling.
	 * <p>
	...
	/**
	 * Sets if the component is currently shown on the screen or not. A call to
	 * <code>show(true)</code> must be made before <code>paint</code>. When the
	 * component is no longer visible call <code>show(false)</code> so that the
	 * component may clean up any resources.
	 * 
	 * @param visible
	 *            when <code>true</code> indicates the component is painted (or
	 ...
	 * events the component must override <code>acceptsInput</code> to return
	 * <code>true</code>.
	 * <p>
	 ...
	 * constants defined in the <code>DeviceScreen</code> class. Note to receive
	 * key events the component must override <code>acceptsInput</code> to
	 ...
	 * <code>selected</code> it will have rounded edges and be highlighted. If
	 * it is not it will have square edges and be slightly inset from (
	 * <code>x</code>, <code>y</code>). The border color, highlight color, and
	 * inner part of the rectangle (background color) all come from the
	 * <code>theme</code>.
	 * 
	 * @param g
	 *            is the <code>Graphics</code> object to be used for rendering
	 ...
	 */
	protected static int paintRect(Graphics g, Theme theme, int x, int y,
