 ...
 * appears as a rectangle that fills from left-to-right as progress toward some
 * known point occurs. There is also a spinner which is animated to show
 * constant progress and is useful when it is unknown how long the operation
 * will last. These are differentiated by <code>setMaxSize</code>; when 0 it is
 * a spinner and when positive it is a progress bar.
 */
public class ProgressBar extends Component {
	...
	/**
	 * The number of times the default font height that the progress bar should
	 * be. Progress bars default to 1.2 (120%) and spinners to 2.0 (200%).
	 */
	 ...
	 * working toward. When <code>current</code> equals <code>max</code> the
	 * progress bar will be full.
	 * <p>
	...
	/**
	 * @return The text that appears above the progess bar. If <code>null</code>
	 *         there is no text.
	 */
	...
	/**
	 * Sets the maximum amount of progress. If this is 0, the total progress is
	 * unknown and this component will be represented by a spinner animation.
	 * 
	 * @param value
	...
	/**
	 * Returns the maximum amount of progress. If this is 0, the total progress
	 * is unknown and this component will be represented by a spinner animation.
	 * 
	 ...
	 *            is the length of the progress bar relative to the width of the
	 ...
	 *            is how tall, relative to the default font, the progress bar
	 *            is. For example 1.2 is 120% the size.
	 */
	...
	/**
	 * Paints a progress bar that shows an operation with a known duration. The
	 * bar fills in left-to-right as progress occurs. The portion on the left
	 * indicating completed progress will take up
	 ...
	 *            selected and <code>false</code> when it is not.
	 */
	protected void paintBar(Graphics g, Theme theme, int x, int y, int width,
			int height, boolean selected) {
		...
		int completedWidth = (int) Math
				.floor(width * percentageCompleted + 0.5);
	 ...
	 *            selected and <code>false</code> when it is not.
	 */
	protected void paintSpinner(Graphics g, Theme theme, int x, int y,
		...
		for (int hour = 1; hour <= 12; hour++) {
			...
			int cx = (int) Math.floor(Math.cos(radians) * toCenter + 0.5);
			int cy = (int) Math.floor(Math.sin(radians) * toCenter + 0.5) * -1; // Y-coordinates
			// flipped
			// on
			// screen
	...
	/**
	 * A task executed at regularly scheduled intervals that forces the spinner
	 * animation to advance to the next frame.
	 */
