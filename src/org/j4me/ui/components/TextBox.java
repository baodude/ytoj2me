...
public class TextBox extends Component {
	 ...
	 * <code>javax.microedition.lcdui.TextBox</code>) allocates a buffer of this
	 * size so it cannot be too big to fit into memory.
	 */
	 ...
	 *             if <code>text</code> is illegal for the current input
	 *             constraints.
	 * @throws IllegalArgumentException
	 *             if <code>text</code> would exceed the current maximum
	 *             capacity.
	 * 
	 * @see #getString()
	 */
	 ...
	 * <code>TextBox</code> are larger than <code>maxSize</code>, the contents
	...
	protected void paintComponent(Graphics g, Theme theme, int width,
		...
		if (contents != null) {
			 ...
			 * else if (isPhoneNumber()) { // Modifiy phone numbers with an area
			 * code. if (display.length() == 10) { StringBuffer builder = new
			 * StringBuffer(15);
			 * 
			 * builder.append("("); builder.append(display.substring(0, 3));
			 * builder.append(") ");
			 * 
			 * builder.append(display.substring(3, 6)); builder.append("-");
			 * 
	...
	protected void select() {
		...
		options &= ~TextField.PASSWORD; // When typing passwords don't * them
		// out; the user needs to see if the
		// phone auto-corrects something it
		// shouldn't
	...
	private final class TextInput extends javax.microedition.lcdui.TextBox
		 ...
		 * @param box
		 *            is the component on <code>parent</code> this input is for.
		 * @param label
		...
		public boolean isBlackBerry() {
			/*
			 * try { Class.forName("net.rim.device.api.ui.UiApplication");
			 * return true; } catch (Throwable e) // ClassNotFoundException, //
