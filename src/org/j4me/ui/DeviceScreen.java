...
/**
 * The <code>DeviceScreen</code> class is a base class for any screen that needs
 * complete control over how it is painted. It is based on and similar to the
 * MIDP <code>Canvas</code> class.
 * <p>
 ...
 * <li><code>getKeyName</code> - The application should define the names to make
 * them consistent across all devices.
 * <li><code>getKeyCode</code> - The key code is passed into the key event
 * methods.
 * <li><code>getGameAction</code> - The game action is passed into the key event
 * methods.
...
public abstract class DeviceScreen {
	 ...
	 * 
	 * @return The <code>javax.microedition.lcdui.Canvas</code> wrapped by this
	 *         screen.
	 */
	 ...
	 * Then this screen's <code>showNotify</code> method will be invoked
	 * followed by the <code>paint</code> method.
	 */
	public void show() {
	 ...
	 * <code>Canvas</code> subclasses may override this method to perform tasks
	 * before being shown, such as setting up animations, starting timers, etc.
	 * The default implementation of this method in class <code>Canvas</code> is
	 * empty.
	 */
	 ...
	 * <code>Canvas</code> subclasses may override this method in order to pause
	 * animations, revoke timers, etc. The default implementation of this method
	 * in class <code>Canvas</code> is empty.
	 */
	 ...
	 * 
	 * @param mode
	 *            is <code>true</code> if the <code>DeviceScreen</code> is to be
	 *            in full screen mode, <code>false</code> otherwise.
	 */
	 ...
	 * 
	 * @return <code>true</code> if in full screen mode (title bar and menu bar
	 *         are hidden); <code>false</code> otherwise.
	 */
	 ...
	 * require turning full screen mode is off (
	 ...
	 * method may result in subsequent call to <code>paint()</code>, where the
	 * passed <code>Graphics</code> object's clip region will include at least
	 * the specified region.
	 * <p>
	 * If the canvas is not visible, or if width and height are zero or less, or
	 * if the rectangle does not specify a visible region of the display, this
	 * call has no effect.
	 * <p>
	 * The call to <code>paint()</code> occurs asynchronously of the call to
	 * <code>repaint()</code>. That is, <code>repaint()</code> will not block
	 * waiting for <code>paint()</code> to finish. The <code>paint()</code>
	 * method will either be called after the caller of <code>repaint()</code>
	 * returns to the implementation (if the caller is a callback) or on another
	 * thread entirely.
	 * <p>
	 * To synchronize with its <code>paint()</code> routine applications can use
	 * <code>serviceRepaints()</code>, or they can code explicit synchronization
	 * into their <code>paint()</code> routine.
	 * <p>
	 ...
	 * <code>Canvas</code>. The X-coordinate is positive right and the
	 * Y-coordinate is positive downwards.
	 * 
	 * @param x
	 *            is the x coordinate of the rectangle to be repainted.
	 * @param y
	 *            is the y coordinate of the rectangle to be repainted.
	 * @param width
	 *            is the width of the rectangle to be repainted.
	 * @param height
	 *            is the height of the rectangle to be repainted.
	 ...
	 * Override <code>Theme.paintBackground</code> to change the background for
	 ...
	 * title has been set through <code>setTitle</code> and the canvas is not in
	 * full screen mode.
	 * <p>
	 ...
	 * override <code>Theme.paintMenuBar</code>. Be careful not to write strings
	 * that are too long and will not fit on the menu bar.
	 * 
	 * @param g
	 *            is the <code>Graphics</code> object to paint with.
	 * @param left
	 ...
	 *            "Cancel" button.
	 * @param highlightLeft
	 *            is <code>true</code> if the menu text <code>left</code> should
	 *            be highlighted to indicate the left menu button is currently
	 *            pressed.
	 * @param right
...
final class CanvasWrapper extends javax.microedition.lcdui.Canvas implements
	 ...
	 * integrate well. However, we try to make this class work so J4ME code can
	 * be embedded within Android applications.
	 * <p>
	 * Android should draw the title bar and menu bar. It should also add the
	 * LCDUI menu which will hook into Android's Menu and Back buttons (similar
	 * to the BlackBerry keyboard layout).
	 */
	 ...
	 * <code>REPEAT_PERIOD</code>. This value will be <code>null</code> if the
	 * user is not holding down any keys.
	 * <p>
	...
	static {
		...
		/*
		 * // Check if running on a BlackBerry. try {
		 * Class.forName("net.rim.device.api.ui.UiApplication"); blackberry =
		 * true; } catch (Throwable e) // ClassNotFoundException,
		 * NoClassDefFoundError { blackberry = false; }
		 * 
		 * // Check if running on IBM's J9 JVM. try {
		 * Class.forName("java.lang.J9VMInternals"); ibmJ9 = true; } catch
		 * (Throwable e) // ClassNotFoundException, NoClassDefFoundError { ibmJ9
		 * = false; }
		 * 
		 * // Check if running on Tao's JVM. if (platform.indexOf("intent") >
		 * -1) { tao = true; } else { tao = false; }
		 * 
		 * // Check if running on Android. try {
		 * Class.forName("android.content.Context"); android = true; } catch
		 * (Throwable e) // ClassNotFoundException, NoClassDefFoundError {
		 * android = false; }
		 */
	}
	...
	/**
	 * Causes the <code>keyRepeated</code> method to fire on devices that do not
	 * natively support it.
	 */
	...
	private int translateKeyCode(int key) {
		...
		if ((key == -6) || (key == -21) || (key == -9)||(key==49)|| (key == -8)) {
	 ...
	 *            when <code>true</code> allows <code>DeviceScreen</code> to use
	 *            the entire screen.
	 * 
	 ...
	 * 
	 * @param c
	 *            is a <code>Command</code> object identifying the command.
	 * @param d
	 *            is the <code>Displayable</code> on which this event has
	 *            occurred. It will always be <code>this</code> screen.
	 * 
	 ...
	 *            method returns; <code>false</code> does it in the normal paint
	 *            cycle.
	 */
	...
	protected void paint(Graphics g) {
		try {
			...
			// problem.
			System.err.println("Unhandled exception in paint for " + master
					+ t.toString());
		}
	}
}
