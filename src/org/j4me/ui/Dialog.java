 ...
 * user input using standard components like the text box. It is similar to the
 * MIDP <code>Form</code> class except that it uses the J4ME control flow and
 * has an OK and Cancel button built in.
 * <p>
 ...
 * components. If you need direct control over the appearance of the screen use
 * the <code>DeviceScreen</code> class.
 * 
...
public abstract class Dialog extends DeviceScreen {
	 ...
	 * <code>Component.acceptsInput()</code>). It is up the component itself to
	 * decide if it should be painted differently when highlighted.
	 */
	 ...
	 * indicate that the current top of the screen (i.e. y == 0) shows the 100th
	 * pixel down in the form. If the first component were 80 pixels tall and
	 * the second 40, 100 would mean the bottom half of the second component
	 ...
	 * calculated. If the components on the form are changed, this will be reset
	 ...
	 * paintings. Recalculating the layout is expensive so we just set this flag
	 * to <code>true</code> many times instead. Then do just a single layout
	 ...
	 * <code>index</code>. Each component on this form with an index greater or
	 ...
	 * @param index
	 *            is where to insert <code>component</code> on the form. It must
	 *            greater than or equal to 0 and less than or equal to the
	 *            number of components already on the form.
	 ...
	 * <code>index</code>. The previous component at that position is discarded.
	 * 
	 * @param component
	 ...
	 * form. If <code>component</code> is found on this form, each component on
	 * the form with an index greater or equal to the <code>component</code>'s
	 ...
	 * specified index is shifted downward to have an index one smaller than the
	 * value it had previously.
	 * 
	 * @param index
	 *            is the index of the component to remove. It must be a value
	 *            greater than or equal to 0 and less than the current number of
	...
	/**
	 * Returns the index of the currently selected component. It is the one the
	 * user can currently enter data into.
	 * 
	 ...
	 *         <code>y</code>) or <code>-1</code> if no component contains it.
	 */
	...
	public void setSelected(int index) {
		...
		int maxScroll = absoluteHeights[absoluteHeights.length - 1]
				- getHeight();
	...
	/**
	 * Gets the index of the given component. Components start at 0 and go to
	 * the total number of components on the form minus 1.
	 * 
	 * @param component
	 *            is the component to get the index of.
	...
	protected synchronized void paint(Graphics g) {
		...
		for (int i = 0; i < absoluteHeights.length - 1; i++) {
			...
			} else // visible
			{
				...
				if (intersects(g, componentX, componentY, componentWidth,
						componentHeight)) {
	 ...
	 */
	protected void paintVerticalScrollbar(Graphics g, int x, int y, int width,
	...
	/**
	 * Sets the margin for the left and right of the screen. This also pads the
	 * top and bottom of the form; however, scrolling can make the text appear
	 * at the very top and bottom of the screen.
	 * 
	 * @param margin
	 ...
	 * 
	 * @param spacing
	 *            is the number of pixels that vertically separates components.
	 *            Values less than 0 are be ignored.
	 */
	...
	/**
	 * Returns the width of the usuable portion of this form. The usable portion
	 * excludes the vertical scrollbar on the right of the screen (if it
	 * exists).
	 * 
	...
	public int getWidth() {
		...
		if (hasVerticalScrollbar()) {
			int scrollbarWidth = UIManager.getTheme()
	 ...
	 * positioning. The form's overall height, the sum of all the components, is
	 * set in the <code>height</code> member variable.
	 * 
	 * @param theme
	...
	private synchronized void calculateLayout(Theme theme, int width, int height) {
		componentWidths = new int[components.size() + 1]; // Dummy element at
		// bottom for end of
		// components
	...
	/**
	 * Returns the next component that <code>highlightedComponent</code> should
	 * be set to. The way it is selected is:
	 * <ol>
	 ...
	 * 
	 * @param down
	 *            when <code>true</code> means the next component with a higher
	 *            index; when <code>false</code> means the next with a lower.
	 * @param maxScroll
	 *            is the maximum number of pixels that can be scrolled to get to
	 *            the next component. For example it may be 90% the height of
	 *            the screen.
	...
	private void scroll(boolean down) {
		...
		int topOfForm = 0;
		int screenHeight = getHeight();
		int bottomOfForm = absoluteHeights[absoluteHeights.length - 1]
				- screenHeight;
			...
			} else // up
			{
				...
				if (currentTop < topOfScreen) {
					...
					if (highlightedComponent == 0) {
						...
						// highlighted one.
						int previousBottom = absoluteHeights[highlightedComponent]
								- spacing;
	...
	protected void pointerPressed(int x, int y) {
		...
		if (hasVerticalScrollbar()) {
			...
			if (x >= scrollbarX) {
				...
				double trackbarPercentage = (double) height
						/ (double) formHeight;
				int trackbarHeight = (int) Math.floor(height
						* trackbarPercentage + 0.5);
				...
				double offsetPercentage = (double) topOfScreen
						/ (double) scrollableHeight;
				int center = rangeStart
