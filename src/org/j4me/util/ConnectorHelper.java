...
public class ConnectorHelper extends Thread {
	 ...
	 * simple time out (<code>IOException</code>). It could also mean we were
	 * not allowed access to the API (<code>SecurityException</code>).
	 */
	...
	public synchronized void run() {
		...
		try {
			connection = (HttpConnection) Connector.open(url, mode, true);
			...
			// SecurityException.
			System.err.println("Problem opening connection to " + url
					+ e.toString());
			exception = e;
		} catch (SecurityException e) {
			System.err.println("Not allowed to open connection" + e.toString());
			exception = e;
		} catch (Exception e) {
			System.err.println("Unknown connection exception" + e.toString());
			exception = e;
		}
	 ...
	 * exception.
	 * 
	 * @return A connection to the device; <code>null</code> if it timed out and
	 *         was unsuccessful.
	 * @throws IOException
	 ...
	 *             prohibited.
	 */
	public synchronized HttpConnection blockUntilConnected()
	...
	public static Connection open(String url, int mode, int timeout)
		...
		HttpConnection connection = thread.blockUntilConnected();
